...
final class AudioDucker: NSObject, AVAudioPlayerDelegate {
    ...
    func play(url: URL, onFinish: (() -> Void)? = nil) {
        do {
            ...
            // Always (re)activate: switching to `.duckOthers` only engages the duck
            // once the session is activated *after* the category change. Skipping it
            // (as an "already active" optimization) left clips silent whenever other
            // audio — Spotify — was playing, which the simulator (no other audio)
            // couldn't reproduce.
            try session.setActive(true)
    ...
    private func restore() {
        if listening {
            try? session.setCategory(.playback, mode: .default, options: [.mixWithOthers])
            try? session.setActive(true)   // re-apply so the ducked audio swells back up
        } else {
            try? session.setActive(false, options: [.notifyOthersOnDeactivation])
        }
    }
